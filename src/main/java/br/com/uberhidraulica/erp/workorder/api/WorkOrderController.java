...
public class WorkOrderController {
 ...
 @PostMapping("/{id}/services") ResponseEntity<Response> add(@PathVariable UUID id,@Valid @RequestBody AddServiceRequest r){return ResponseEntity.status(HttpStatus.CREATED).body(Response.from(application.addService(id,r.serviceId(),r.price())));}
 ...
 public record Response(UUID id,Long number,UUID customerId,UUID vehicleId,long entryMileage,Instant openedAt,WorkOrder.Status status,List<ServiceResponse> services,List<ProductResponse> products){static Response from(WorkOrder w){return new Response(w.id(),w.number(),w.customerId(),w.vehicleId(),w.entryMileage(),w.openedAt(),w.status(),w.services().stream().map(ServiceResponse::from).toList(),w.products().stream().map(ProductResponse::from).toList());}}
 public record ServiceResponse(UUID id,UUID serviceId,String name,String description,BigDecimal basePrice,int warrantyDays,Instant addedAt,String priceSource){static ServiceResponse from(WorkOrder.ServiceItem i){return new ServiceResponse(i.id(),i.serviceId(),i.name(),i.description(),i.basePrice(),i.warrantyDays(),i.addedAt(),i.priceSource());}}
 public record ProductResponse(UUID id,UUID productId,String description,String internalCode,String unit,BigDecimal quantity,BigDecimal unitPrice,Instant addedAt){static ProductResponse from(WorkOrder.ProductItem i){return new ProductResponse(i.id(),i.productId(),i.description(),i.internalCode(),i.unit(),i.quantity(),i.unitPrice(),i.addedAt());}}
}
