...
public class WorkOrderExceptionHandler {
 /** Estados do catálogo ou do vínculo que impedem a operação sem que o pedido esteja malformado. */
 private static final Set<String> CONFLICTS=Set.of("VEHICLE_CUSTOMER_MISMATCH","PRODUCT_INACTIVE","PRODUCT_WITHOUT_SALE_PRICE");
 @ExceptionHandler(WorkOrderException.class) ResponseEntity<ErrorResponse> handle(WorkOrderException e){HttpStatus s=e.code().endsWith("NOT_FOUND")?HttpStatus.NOT_FOUND:CONFLICTS.contains(e.code())?HttpStatus.CONFLICT:HttpStatus.BAD_REQUEST;return ResponseEntity.status(s).body(new ErrorResponse(e.code(),e.getMessage(),List.of()));}
