...
public class JpaWorkOrderRepositoryAdapter implements WorkOrderRepositoryPort {
 ...
 public List<WorkOrder> findByCustomerId(UUID id){return orders.findByCustomerIdOrderByOpenedAtDescIdAsc(id).stream().map(this::map).toList();}
 public List<WorkOrder> findByVehicleId(UUID id){return orders.findByVehicleIdOrderByOpenedAtDescIdAsc(id).stream().map(this::map).toList();}
 public WorkOrder.ServiceItem addService(UUID orderId,WorkOrder.ServiceItem i){WorkOrderServiceEntity e=new WorkOrderServiceEntity();e.id=i.id();e.workOrderId=orderId;e.serviceId=i.serviceId();e.serviceName=i.name();e.serviceDescription=i.description();e.basePrice=i.basePrice();e.warrantyDays=i.warrantyDays();e.addedAt=i.addedAt();e.priceSource=i.priceSource();return map(items.saveAndFlush(e));}
 ...
 private WorkOrder.ServiceItem map(WorkOrderServiceEntity e){return new WorkOrder.ServiceItem(e.id,e.serviceId,e.serviceName,e.serviceDescription,e.basePrice,e.warrantyDays,e.addedAt,e.priceSource);}
