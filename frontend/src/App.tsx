...
export default function App(){const{session,loading}=useAuth();return <Routes><Route path="/orcamento/:token" element={<PublicQuotePage/>}/><Route path="/login" element={loading?<main className="center-state">Carregando sessão…</main>:session?<Navigate to={session.mustChangePassword?'/alterar-senha':'/ordens-servico'} replace/>:<LoginPage/>}/><Route path="/alterar-senha" element={loading?<main className="center-state">Carregando sessão…</main>:!session?<Navigate to="/login" replace/>:<PasswordChangePage/>}/><Route element={<ProtectedRoute/>}><Route element={<AppShell/>}><Route path="/clientes" element={<CustomersPage/>}/><Route path="/clientes/novo/pf" element={<CustomerFormPage type="PF"/>}/><Route path="/clientes/novo/pj" element={<CustomerFormPage type="PJ"/>}/><Route path="/veiculos" element={<VehiclesPage/>}/><Route path="/veiculos/novo" element={<VehicleFormPage/>}/><Route path="/servicos" element={<ServicesPage/>}/><Route path="/servicos/novo" element={<ServiceFormPage/>}/><Route path="/produtos" element={<ProductsPage/>}/><Route path="/produtos/novo" element={<ProductFormPage/>}/><Route path="/produtos/:id" element={<ProductFormPage/>}/><Route path="/ordens-servico" element={<WorkOrdersPage/>}/><Route path="/ordens-servico/nova" element={<WorkOrderFormPage/>}/><Route path="/ordens-servico/:id" element={<WorkOrderDetailPage/>}/><Route path="/ordens-servico/:id/orcamentos" element={<QuotesPage/>}/><Route path="/ordens-servico/:id/orcamentos/:quoteId" element={<QuoteDetailPage/>}/></Route></Route><Route path="/" element={<Navigate to={session?'/ordens-servico':'/login'} replace/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes>}
